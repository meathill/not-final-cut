...
async function stt(wav) {
  ...
  let task = {
    appkey: aliyunAppKey,
    file_link: urlPrefix + filename, // eslint-disable-line camelcase
    version: '4.0',
    enable_words: true, // eslint-disable-line camelcase
  };
  ...
  console.log('[NFC]   creating convert task...');
  ...
  console.log('[NFC]   start converting');
    ...
    new Promise((resolve, reject) => {
      const timer = setInterval(async() => {
        ...
        if (statusText === 'RUNNING' || statusText === 'QUEUEING') {
          // 继续轮询，注意间隔周期。
          console.log('[NFC]     still in converting...');
          return;
        }

        if (statusText === 'SUCCESS' || statusText === 'SUCCESS_WITH_NO_VALID_FRAGMENT') {
          console.log('[NFC]   converting successfully');
          resolve(response.Result);
        } else {
          console.log('[NFC]   failed to convert speech to text');
          reject();
        }
        // 退出轮询
        clearInterval(timer);
